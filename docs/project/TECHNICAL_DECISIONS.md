...
- **Rejected diagnostic path**: Alpha53 downstream call-stack signatures were identical. Alpha54 then instrumented the existing Objective-C `NSURLSession dataTaskWithRequest:/dataTaskWithURL:/uploadTaskWithRequest:` creation selectors, but the real-device trace emitted **zero `REFRESH-CREATE` records** while 14 refresh-relevant `REFRESH-PATH` records were observed. Those specific creation selectors are therefore not the upstream path for these semantic host requests in the tested runtime. The exact higher-level Foundation/Swift API remains `Unknown / Unverified`; do not guess it.
- **Next evidence direction**: If investigation continues, observe public `UINavigationController` stack mutation entry points diagnostically during the existing user-started trace, recording only before/after bounded class composition and sanitized caller evidence. Observation does not authorize calling those mutation methods as a refresh mechanism.
...
- **Validation level**: alpha52/53/54 real-device traces. Production refresh remains unverified.
