...
- **Evidence**: User repeatedly observed plugin `1/3` after 429; source proved one tap could create up to four requests. Alpha51 removed 429 retries and added exact-ID Sync guards. Alpha51–54 captured traces did not contain 429, so terminal-429 behavior is not yet device exercised by trace evidence.
...
- **Evidence**: Alpha51 trace showed three route attempts each produced only another same-ID detail GET without visible rebuild. Alpha53 and alpha54 traces exercised the alpha52 suppression logic: after route attempt 0 produced one same-ID detail GET and no UI rebuild, they emitted `stop-repeat-route` and ended without attempts 1/2.
...
- **Decision**: Treat exact `conversation/init → conversation/prepare → conversation detail` traffic as evidence that the official client has already entered a genuine conversation-navigation state. Do **not** infer that manually originating those requests would cause the host SwiftUI/UI state to transition or rebuild. Navigation count/composition is evidence of host state, not authorization to recreate or mutate that state.
- **Evidence — alpha52/53**: Genuine exact A→B→A navigation emitted exact init followed by prepare/detail within ~125 ms and showed public `SwiftUI.UIKitNavigationController navCount=3`; failed same-current custom-route Sync/Reload emitted detail only, no exact init/prepare, no UI rebuild, and `navCount=1`.
- **Evidence — alpha54**: Trace `1995A79E-71DF-4EBC-BB1E-A61D48871FD2` captured ID-less `conversation/init` + ID-less prepare staging while public navigation depth was 2, followed by exact target init/prepare/detail at depth 3. Returning to A retained depth 3 for exact navigation. Same-current custom-route detail appeared at depth 1. This strengthens the conclusion that the exact network sequence follows a host navigation-state transition rather than owning it.
- **Rejected diagnostic path**: Alpha53 downstream call-stack signatures were identical. Alpha54 then instrumented the existing Objective-C `NSURLSession dataTaskWithRequest:/dataTaskWithURL:/uploadTaskWithRequest:` creation selectors, but the real-device trace emitted **zero `REFRESH-CREATE` records** while refresh-relevant `REFRESH-PATH` records were observed. Those specific creation selectors are therefore not the upstream path for these semantic host requests in the tested runtime. The exact higher-level Foundation/Swift API remains `Unknown / Unverified`; do not guess it.
- **Alpha55 diagnostic refinement**: `0.1.0-alpha55-navigation-mutation-trace` passively swizzles only public `UINavigationController` mutation entry points (`setViewControllers` plus push/pop family) and emits `NAV-MUTATION` only while the user-started trace is active and the bounded controller count/class composition actually changes. Records include before/after structure and sanitized caller symbols. This is observation only; no host navigation mutation is originated by the enhancer.
- **Rejected / do-not-repeat**: Do not replay init/prepare requests, force a three-controller stack, hard-code observed Swift controller classes, use UIKit pop/push as a fallback, reintroduce History/sidebar navigation, alternate IDs, guessed `/resume`, extra route variants, timers or watchdogs.
- **Validation level**: alpha52/53/54 real-device traces; alpha55 **Code written → CI passed → Artifact produced; runtime pending**. Production refresh remains unverified.
