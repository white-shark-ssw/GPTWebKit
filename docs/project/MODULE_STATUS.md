...
| `ChatGPTEnhancer/Sources/Diagnostics` | Active / Experimental | diagnostics/probe modules | Alpha54 is real-device partially tested: ID-less staging appeared at nav depth 2, genuine exact navigation at depth 3, failed same-current route detail at depth 1, and `REFRESH-CREATE` remained zero. Next evidence should follow public navigation-stack mutations rather than the rejected selected task-creation selectors. Diagnostic presence is not proof of a production refresh mechanism. |
