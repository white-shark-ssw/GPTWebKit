...
static void CERecoveryCancelStaleStreamTasks(NSDate *cutoff, NSString *conversationID, NSUInteger generation) {
    NSURLSession *session = [CENetworkObserver shared].requestSession;
    if (!session || !cutoff || !conversationID.length) return;
...
static void CERecoveryCheckServer(NSString *conversationID, NSDate *cutoff, NSUInteger generation, NSUInteger attempt) {
    ...
    if (![[CEAPIClient shared] isReady]) {
        if (attempt < 2) dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(2.0 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{ CERecoveryCheckServer(conversationID, cutoff, generation, attempt + 1); });
        return;
    }
    ...
    [[CEAPIClient shared] getPath:path progress:nil completion:^(NSData *data, NSHTTPURLResponse *response, NSError *error) {
        ...
        if (!error && response.statusCode >= 200 && response.statusCode < 300 && CERecoveryConversationFinished(data)) {
            CERecoveryCancelStaleStreamTasks(cutoff, conversationID, generation);
            return;
        }
        if (attempt < 2) {
...
static void CERecoveryInstall(void) {
    ...
    dispatch_once(&once, ^{
        ...
        [center addObserverForName:UIApplicationDidBecomeActiveNotification object:nil queue:NSOperationQueue.mainQueue usingBlock:^(__unused NSNotification *note) {
            ...
            dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(1.25 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{ CERecoveryCheckServer(conversationID, cutoff, generation, 0); });
        }];
