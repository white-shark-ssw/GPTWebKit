...
static void CEShowLoadedToastWhenReady(NSUInteger attempt) {
    if (CEKeyWindow()) { CEShowMessage(@"ChatGPTEnhancer alpha52-sync-refresh-handoff 已加载"); return; }
...
static void CEStartEnhancer(void) {
    ...
    CERecoveryDiagnosticMark(@"PLUGIN START alpha52-sync-refresh-handoff");
    @try {
        ...
        NSLog(@"[ChatGPTEnhancer] alpha52-sync-refresh-handoff started for %@ %@", NSBundle.mainBundle.bundleIdentifier, [NSBundle.mainBundle objectForInfoDictionaryKey:@"CFBundleShortVersionString"] ?: @"unknown");
