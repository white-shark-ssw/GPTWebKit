...
static void CEShowLoadedToastWhenReady(NSUInteger attempt) {
    if (CEKeyWindow()) { CEShowMessage(@"ChatGPTEnhancer alpha36-model-aware-usage 已加载"); return; }
...
static void CEStartEnhancer(void) {
    ...
    CERecoveryDiagnosticMark(@"PLUGIN START alpha36-model-aware-usage");
    @try {
        ...
        NSLog(@"[ChatGPTEnhancer] alpha36-model-aware-usage started for %@ %@", NSBundle.mainBundle.bundleIdentifier, [NSBundle.mainBundle objectForInfoDictionaryKey:@"CFBundleShortVersionString"] ?: @"unknown");
