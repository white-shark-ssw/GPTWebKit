...
static void CEShowLoadedToastWhenReady(NSUInteger attempt) {
    if (CEKeyWindow()) { CEShowMessage(@"ChatGPTEnhancer alpha44-current-conversation-guard 已加载"); return; }
...
static void CEStartEnhancer(void) {
    ...
    CERecoveryDiagnosticMark(@"PLUGIN START alpha44-current-conversation-guard");
    @try {
        ...
        NSLog(@"[ChatGPTEnhancer] alpha44-current-conversation-guard started for %@ %@", NSBundle.mainBundle.bundleIdentifier, [NSBundle.mainBundle objectForInfoDictionaryKey:@"CFBundleShortVersionString"] ?: @"unknown");
